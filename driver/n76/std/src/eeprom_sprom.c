...
void WriteDataToSPOnePage(uint8_t u8_addr, uint8_t *pDat, uint16_t num)
{
    ...
    {
#if defined __C51__
        xd_tmp0[i] = Read_SPROM_BYTE((uint8_t code *)i);
#elif defined __ICC8051__
        xd_tmp0[i] = Read_SPROM_BYTE((uint8_t __code *)i);
#elif defined __SDCC__
        xd_tmp0[i] = Read_SPROM_BYTE((uint8_t __code *)i);
#endif
